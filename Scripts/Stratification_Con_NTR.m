...
clear
clc
load('C:\Users\pr109704\OneDrive - University of Central Florida\Analysis_DW_5d_TC_350km_Philli_airport_AORC_Chelsa\Two Way Sampling\Con_NTR\Maximum_RF_events_for_each_POT_NTR.mat');
load('C:\Users\pr109704\OneDrive - University of Central Florida\Analysis_DW_5d_TC_350km_Philli_airport\Cyclone_Track\Cyclone_Track_data_from_1850_new');
...
%distance2 = km2deg(500);
Tropical_Cyclones = struct;
ET_Cyclones = struct;

%% One event will be manually transfered in to the List date: 2009/09/19
...
for i = 1:48
    ...
    for j =1:leng
        ...
        if length(index)>0
            if Time_ntr>731842 && Time_ntr<731844 
               x=2
...
for k =1:6
    TC500(k,1) = Tropical_Cyclones(16).Event(k).Time_NTR;
    TC500(k,2) = Tropical_Cyclones(16).Event(k).NTR;
    TC500(k,3) = Tropical_Cyclones(16).Event(k).Time_RF;
    TC500(k,4) = Tropical_Cyclones(16).Event(k).RF;
end