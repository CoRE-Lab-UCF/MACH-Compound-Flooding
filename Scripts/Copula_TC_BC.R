...
Copula_TC_BC<-function(Data, Data_Con1, Data_Con2, Data_Con1_M, Data_Con2_M, u1=NA, u2=NA, Thres1, Thres2, Copula_Family1, Copula_Family2, Marginal_Dist1, Marginal_Dist2, Marginal_Dist1_Par=NA, Marginal_Dist2_Par=NA, Con1,Con2, GPD_con1, GPD_con2, mu=365.25, GPD_Bayes=FALSE, Decimal_Place=2, RP, Interval=10000, End=TRUE, Resolution="Low", x_lab="Rainfall (mm)",y_lab="O-sWL (mNGVD 29)",x_lim_min = NA,x_lim_max = NA,y_lim_min = NA,y_lim_max = NA,Isoline_Probs="Sample", N=10^6,N_Ensemble=0,Sim_Max=10,Plot_Quantile_Isoline=FALSE,Isoline_Type="Combined",EL_con1,EL_con2){
...
#  }
  ...
  marginal_non_cond_1 <-fitdistr(Con_NTR[,2], "gamma")
  cdf_non_con1 = pgamma(Pgrid[,2], marginal_non_cond_1$estimate[1], rate = marginal_non_cond_1$estimate[2], lower.tail = TRUE,log.p = FALSE)
  ...
  cop.sample1.non.con<-qgamma(sample[,2], shape = as.numeric(marginal_non_cond_1$estimate[1]), rate = as.numeric(marginal_non_cond_1$estimate[2]))
  cop.sample1<-data.frame(cop.sample1.con,cop.sample1.non.con)
  colnames(cop.sample1)<-c("Var1","Var2")
  ...
  write.csv(cop.sample1, "TC_Cop_Sample_Con_NTR.csv", row.names=FALSE)
  write.csv(cop.sample2, "TC_Cop_Sample_Con_RF.csv", row.names=FALSE)
  
  
  
  #Combine the data frames containg the samples from two copulas (on the original scale)
  cop.sample<-rbind(cop.sample1,cop.sample2)
  write.csv(cop.sample, "TC_Cop_Sample.csv", row.names=FALSE)
  ...
  write.csv(RP_TC, "RP_TC.csv", row.names=FALSE)
  #########################################################################################
