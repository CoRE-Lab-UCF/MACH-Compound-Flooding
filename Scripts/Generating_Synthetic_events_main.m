...
% Combinign Scaled NTR with MSL and Tide ( The observed NTR pak lag time with next high tide will be used)

% RP_Comb = Combined retunr period of the event
% len_TC_sim =  Number of TC simulatins from copulas
% len_non_TC_sim =  Number of non-TC simulations copulas
% NTR_scaled = Scaled NTR data (vector should be come from previous function)
% Monthly_Tidal_curves = Tidal signal segments around each high tide seperated for each month
% MSL = MSL distribution in seperated to each calender month
% numb_TC =  Number of TCs recorded
% numb_ETC =  Number of non-TCs reocorded
% TC_monthly_dis =  Distributoin of frequency of occurence of TCs in monthly basis
% ETC_monthly_dis =  Distributoin of frequency of occurence of non-TCs on monthly basis
% Storm_Tide_hydrograph = A strcuture file containing storm tide hydrographs and other paramters
...
% RF_fields = the evctor of observed Rf fields (3D metrix, 2d array of RF grid points over the catchemnt for
% each hour)
% RF_Acc = Selected RF Accumulation time
% DES_RF = Target RF values simulated from fitted copulas
%[RF_Scaled]=Scaling_RF_events(RF_Acc,DES_RF,RF_Data,RF_fields,RF_select)
[RF_Scaled]=Scaling_RF_events(Acc,DES_RF(i),RF_Data,RF_fields,RF_select);
