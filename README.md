...
The script uses the Peak over threshold (POT) approach to define events over thresholds for non-tidal residuals (NTR).
...
The script uses the Peak over threshold (POT) approach to define events over thresholds for basin average rainfall. The code calculates the threshold exceedances for rainfall accumulation times from 1 to 48.
...
The script finds the maximum accumulated rainfall (for all the hourly accumulations from 1 to 48) within a given time window around the selected POT event when conditioning NTR.
...
4. 1 Conditioned on RF
The script finds the maximum NTR (for all the selected accumulation time) within a given time window around the selected POT event when conditioning RF.
...
## 5. Stratification
The two conditional samples are stratified in to two sets as: 1. The events caused by Tropical cyclones 2. The events that were not caused by tropical cyclones. The Hurdat 2  data set is used to identify the events induced by tropical cylcones. 
...
## 7. Fitting Copulas
The two stratified conditional samples are fitted in to different Copula families to check the most appropriate copula family. This is done based on the AIC and the “Multihazard R package” was used (https://rdrr.io/github/rjaneUCF/MultiHazard/f/README.md). 

## 8. Bivariate analysis and estimating Joint probability distributions
The two stratified conditional samples are used to:
...
The separately estimated return periods (annual exceedance probabilities) of TC samples and non-TC samples are combined:
